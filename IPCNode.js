	...
	* Outgoing stuff *
	...
	IPCNode.prototype._onInfoRequest=function() {
		var ids=Array.prototype.slice.call(arguments),
			...
			newMarshalled=[],
			requested=[],
			i,id,object,currentInfo,currentProperties;
		...
		requested.forEach(function(x) {
			...
			currentProperties=self._marshalProperties(object,marshalLocalObjectCallback);
			if (currentProperties) {
				currentInfo[1]=currentProperties;
			}
			objectTable[id]=currentInfo;
		});
...
if (typeof(exports)==="object" && typeof(require)==="function") {
	IPCNodeModule(exports,require("./IDProvider").IDProvider,require("events").EventEmitter);
} else if (typeof(window)==="object") {
	if (typeof(window.EventEmitter)!=="function") {
		throw new Error("No EventEmitter implementation found");
	}
	if (typeof(window.IDProvider)!=="function") {
		throw new Error("No IDProvider implementation found");
	}
	IPCNodeModule(window,window.IDProvider,window.EventEmitter);
}
})();
